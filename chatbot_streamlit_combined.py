...
def display_chatbot_page():
    ...
    with st.expander("Initialize the LLM Model"):
        ...
        with st.form("setting"):
            row_1 = st.columns(3)
            with row_1[0]:
                token_value = os.getenv('HUGGINGFACEHUB_API_TOKEN')
                masked_token = f"{'*' * len(token_value)}" if token_value else ""
                text = st.text_input("Hugging Face Token (No need to insert)", type='password', value=masked_token)
